...
#git clone https://github.com/catonrug/ditto-detect.git && cd ditto-detect && chmod +x check.sh && ./check.sh
...
filelist=$(wget -qO- "$download" | sed "s/http/\nhttp/g;s/download/download\n/g" | grep "^http.*download$" | grep -v "DAO\|Debug\|Source\|readme" | sort | uniq | sed '$alast line')
...
case "$filename" in
*Portable*64bit*zip)
type=$(echo "Portable (64-bit)")
;;
*Setup*64bit*exe)
type=$(echo "(64-bit)")
;;
*Portable*zip)
type=$(echo "Portable (32-bit)")
;;
*Setup*exe)
type=$(echo "(32-bit)")
;;
